    ...
    components: [
        {
            ...
            components: [
                    ...
                    components: [
                        {
                            ...
                            dataUrl: {
                                ...
                                debounce: 200,
                                method: 'post',
                                data: { count: 100, devName: '{{stationDesc}}??undefined', devType: ['substation'], regionIdList: REGION_ID ? [REGION_ID] : undefined, subareaIdList: AREA_ID ? [AREA_ID] : undefined },
                                transform(res) {
                                    _model.stationListData = res.data.map((item) => ({ name: item.stName, value: item.stId }));
                                }
                            }
                        },
                        {
                        ...
                        },
                        {
                            ...
                            onclick() {
                                exportPtrOverload('heavyload', { devName: _msgr.get('devName'), regionIdList: REGION_ID ? [REGION_ID] : undefined, subareaIdList: AREA_ID ? [AREA_ID] : undefined, stIdList: _model.stIdList ?? undefined });
                            }
                        }
                    ]
                },
                {
                    ...
                    ref: 'table',
                    buildIf: '{{heavyloadTableData}}.length>0',
                    dataDef: [
                        {
                            cap: '变电站',
                            sortable: false,
    ...
    ajaxCall(
        'getPtrOverloadPage',
        {
            ...
            success(res) {
                ...
                _model.vars.heavyloadTableData = newData;
            },
            params: { pageSize: 200, pageIndex: 1, status: 1, ...params },
            uniqId: jam.genUUID(),
            useMock: false
        },
        false
...
export const exportPtrOverload = (type, params) => {
    // nutmeg.success('正在导出数据，请稍等...');
    nutmeg.success('正在导出数据，请稍等...', { duration: 1000 });
    exportExcel(
        urlConfig.exportPtrOverload.url,
        {
            pageIndex: 1,
            pageSize: 200,
            status: transformObj[type].status,
            ...params
        },
        transformObj[type].name + '.xlsx'
    );
};
